...
public class Program
{
    public static void Main()
    {
        ...
        while (true)
        {
            ...
            if (input.Contains("-"))
            {
                ...
                else people.FirstOrDefault(p => p.Birthdate == parent.Birthdate).AddChild(child);
            }
            else
            {
                var tokens = input
                ...
                if (!isExistingPerson) people.Add(new Person(name, birthdate));
            }
        }
    ...
    private static Person CreatePerson(string personParam)
    {
        ...
        if (IsDate(personParam)) person.Birthdate = personParam;
        else person.Name = personParam;
        return person;
    }
